import {initialState,signal,positionPlan,openPosition,closePosition,applyFunding,grossPnl,liquidationPrice,snapshot,STRATEGIES,round} from './engine.mjs';
...
export function replay(rows,marks,rates,settings,meta,{start=65,end=rows.length,disabled=null}={}) {
  ...
  for(let i=start;i<end;i++) {
    const bar=rows[i],mark=mm.get(bar.t);let p=a.positions[0];
    if(!p) {
      const sig=signal(rows.slice(Math.max(0,i-160),i),settings,disabled);
      const q={mark:mark.o,bid:bar.o*(1-0.0001),ask:bar.o*(1+0.0001),time:bar.t};
      const plan=positionPlan(a,sig,q,meta,bar.t);
      if(plan.accepted)p=openPosition(a,plan,`bt-${bar.t}`);
    }
    if(p) {
      ...
      else if(p.side===1?mark.l<=p.stop:mark.h>=p.stop){reason='ZARAR_DURDUR';exit=p.side===1?Math.min(bar.o,p.stop):Math.max(bar.o,p.stop);}
      ...
      }else{applyFunding(a,p,due,bar.end);p.mark=mark.c;p.markTime=bar.end;}
    }
...
export function laboratory(rows,marks,rates,settings,meta) {
  ...
  return {symbol:meta.symbol,candleCount:rows.length,from:rows[65].t,to:rows.at(-1).end,parameters:settings,baseline:compact(baseline),folds,contribution,
    limitations:['Tek coin testi; coinler arası portföy ve korelasyon testi değildir.','Evren bugünkü sözleşmelerden seçilir; geçmişte kapanmış sözleşmeler kapsanmaz.','Tasfiye, sabit bakım teminatı oranıyla yaklaşık modellenir; Binance hesap sonucu değildir.','Geçmiş emir defteri yok: her yönde 1 baz puan alış-satış farkı varsayılır.','Mum içi sıralama bilinmediğinde tasfiye/stop önceliklidir; fonlama belirsizliği ihtiyatlı hesaplanır.','Her doğrulama penceresi ayrı 1.000 USDT ile başlar; sonuçlar tek portföy eğrisi gibi toplanmaz.','Optimizasyon sonuçları otomatik olarak ayarlara uygulanmaz.']};
}
