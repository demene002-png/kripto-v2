...
const names={YUKSELIS:'Yükseliş',DUSUS:'Düşüş',YATAY:'Yatay',ASIRI_OYNAK:'Aşırı oynak',TASFIYE:'Sanal tasfiye',ZARAR_DURDUR:'Zararı durdur',KAR_AL:'Kâr al',MANUEL:'Manuel kapatma',TEST_SONU:'Test sonu',BEKLE:'Bekle',UYGUN:'Koşullar uygun',ACILDI:'Pozisyon açıldı',trend:'Eğilim',momentum:'İvme',breakout:'Kırılım',pullback:'Geri çekilme'};
let cfg,session,state,revision,dirty=false,settingsRevision,busy=false,historyRows=[],historyCursor=null;
...
$('#history-more').onclick=async()=>{try{const r=await api('/api/v2-app?history=1'+(historyCursor?'&before='+encodeURIComponent(historyCursor):''));historyRows.push(...r.rows.map(x=>x.payload.trade));if(r.rows.length)historyCursor=r.rows.at(-1).created_at;else notice('Daha eski işlem yok.');renderHistory();}catch(e){notice(e.message);}};
