...
test('Yeni test 1000 USDT ile başlar; kapanış geçmişi ve ayarlar korunur',()=>{const {a,p}=setup();closePosition(a,p.id,101,'MANUEL',now+1);const trade=a.trades[0],settings=a.settings;resetTestAccount(a,now+2);assert.equal(a.balance,1000);assert.equal(a.peak,1000);assert.equal(a.dayStartEquity,1000);assert.equal(a.trades[0],trade);assert.equal(a.settings,settings);assert.deepEqual(a.positions,[]);assert.deepEqual(a.pendingEntries,[]);assert.equal(a.curve.length,1);assert.equal(a.events.at(-1).kind,'RESET');});
