...
import {initialState,DEFAULTS,eligible,validateSettings,features,positionPlan,openPosition,createPendingEntry,observePendingEntry,pendingConfirmationReasons,closePosition,applyFunding,equity,liquidationPrice,manageQuote,entryBlocks,correlation,dayKey,resetTestAccount} from '../core/engine.mjs';
...
test('Yeni test 1000 USDT ile başlar; kapanış, tarama geçmişi ve ayarlar korunur',()=>{const {a,p}=setup();closePosition(a,p.id,101,'MANUEL',now+1);a.signals.push({symbol:'BTCUSDT',time:now,action:'BEKLE'});const trade=a.trades[0],signal=a.signals[0],settings=a.settings;resetTestAccount(a,now+2);assert.equal(a.balance,1000);assert.equal(a.peak,1000);assert.equal(a.dayStartEquity,1000);assert.equal(a.trades[0],trade);assert.equal(a.signals[0],signal);assert.equal(a.settings,settings);assert.deepEqual(a.positions,[]);assert.deepEqual(a.pendingEntries,[]);assert.equal(a.curve.length,1);assert.equal(a.events.at(-1).kind,'RESET');});
test('Açık pozisyon varken test bakiyesi değiştirilemez',()=>{const {a}=setup();assert.throws(()=>resetTestAccount(a,now+1),/Önce tüm açık pozisyonları kapatın/);assert.notEqual(a.balance,1000);});
